...
import { TemplateCategory, ThreeDCategory, LottieCategory, LottiePreview } from './ui/LibraryPanels';
import { clampDraggedLayerPosition } from './systems/canvasEngine';
import { shouldBroadcastPresence, buildPresencePayload } from './systems/syncSystem';
import { generateSVGMarkup } from './systems/exportSystem';
...
const ImageEditor = ({
    ...
    const lastSyncedTimestamp = useRef(0); // Track last sync timestamp we sent
    ...
    // Update the pages array from current layers - reduced delay for faster sync
    useEffect(() => {
        const timer = setTimeout(() => {
            setPages(prev => prev.map(p => p.id === activePageId ? { ...p, layers, canvasSize } : p));
        }, 100); // Update global pages state every 100ms for near real-time sync
        ...
        const initFirebaseSync = async () => {
            ...
            try {
                ...
                syncUnsubscribe.current = FirebaseSyncService.initSync(activeDesignId, (remoteData) => {
                    ...
                    // Check if this update is from another browser (not our own)
                    const remoteTimestamp = remoteData.lastModified || 0;
                    const remoteInstanceId = remoteData.lastUpdatedBy;
                    const isFromOtherBrowser = remoteInstanceId && remoteInstanceId !== localInstanceId;
                    const isRemoteUpdate = isFromOtherBrowser || remoteTimestamp > lastSyncedTimestamp.current + 100;

                    console.log('[FirebaseSync] 🔵 Received update:', {
                        pagesCount: remoteData.pages?.length,
                        layersCount: remoteData.pages?.[0]?.layers?.length,
                        designId: activeDesignId,
                        isFromOtherBrowser,
                        remoteInstanceId,
                        ourInstanceId: localInstanceId,
                        isRemoteUpdate,
                        remoteTimestamp,
                        ourLastTimestamp: lastSyncedTimestamp.current
                    });

                    // Apply remote updates from other browsers
                    if (isRemoteUpdate || !lastSyncedTimestamp.current) {
                        console.log('[FirebaseSync] ✅ Applying remote update from:', remoteInstanceId || 'unknown');
                        ...
                        console.log('[FirebaseSync] ⏭️ Skipping our own update');
                    }
                    ...
                    // Short delay - just enough to prevent immediate echo back
                    setTimeout(() => { isReceivingSync.current = false; }, 200);
                });
    ...
    useEffect(() => {
        ...
        syncTimeoutRef.current = setTimeout(async () => {
            const currentState = {
                ...
                lastUpdatedBy: localInstanceId // Track which browser sent this
            };

            try {
                console.log('[FirebaseSync] 🟢 Sending update to Firebase:', { layersCount: layers.length, designId, instanceId: localInstanceId });
                await FirebaseSyncService.updateDesign(designId, currentState);
                lastSyncedTimestamp.current = currentState.lastModified; // Track what we sent
                console.log('[FirebaseSync] ✅ Update sent successfully');
            } catch (error) {
                console.error('[FirebaseSync] Update failed:', error);
            }
        }, 300); // Reduced to 300ms for near real-time sync
    ...
    const handleDelete = (layerId) => {
        ...
        // Trigger immediate sync for real-time collaboration
        syncLayersToFirebase(nextLayers);
        setContextMenu(null);
    };
    ...
    const handleLayerOrder = (layerId, action) => {
        ...
        syncLayersToFirebase(newLayers);
        setContextMenu(null);
    };
    ...
    const addLayerWithSync = (newLayer) => {
        ...
        // Trigger immediate sync to Firebase for real-time collaboration
        if (designId && !isReceivingSync.current && hasLoadedInitialData.current) {
            const currentState = {
                pages: pages.map(p => p.id === activePageId ? { ...p, layers: nextLayers, canvasSize } : p),
                activePageId,
                canvasSize,
                adjustments,
                lastModified: Date.now()
            };
            FirebaseSyncService.updateDesign(designId, currentState).catch(err => {
                console.error('[Immediate Sync] Failed:', err);
            });
        }
    };

    // Helper to sync layer changes to Firebase immediately
    const syncLayersToFirebase = (nextLayers) => {
        if (designId && !isReceivingSync.current && hasLoadedInitialData.current) {
            const currentState = {
                pages: pages.map(p => p.id === activePageId ? { ...p, layers: nextLayers, canvasSize } : p),
                activePageId,
                ...
                adjustments,
                lastModified: Date.now(),
                lastUpdatedBy: localInstanceId
            };
            console.log('[Immediate Sync] 🟡 Sending layers update:', { layersCount: nextLayers.length, designId, instanceId: localInstanceId });
            FirebaseSyncService.updateDesign(designId, currentState)
                .then(() => {
                    lastSyncedTimestamp.current = currentState.lastModified;
                })
                .catch(err => {
                    console.error('[Immediate Sync] Failed:', err);
                });
    ...
    const handleMouseDown = (e, layer) => {
        if (isViewOnly || layer.isLocked || layer.id === 'background-layer') return;
        if (editingLayerId === layer.id && !e.altKey) return;
        if (e.target?.isContentEditable && editingLayerId === layer.id && !e.altKey) return;
        e.preventDefault();
        e.stopPropagation();
        ...
        // Broadcast selection
        if (designId && user?.uid) {
            FirebaseSyncService.updatePresence(designId, user.uid, buildPresencePayload({
    ...
    const handleMouseUp = React.useCallback(() => {
        if (dragRef.current.isDragging || resizing || isResizingCrop) {
            ...
            // Sync layer position changes to Firebase for real-time collaboration
            syncLayersToFirebase(layers);
        }
    ...
    useEffect(() => {
        const applyDragFrame = () => {
            ...
            if (!evt || !containerRef.current || !dragRef.current.isDragging || !dragRef.current.layerId) return;
            ...
            setLayers(prev => prev.map(l => {
                ...
                if (isPanningContent && (l.shapeType === 'image' || l.type === 'frame')) {
                    const moveX = evt.movementX / (zoom || 1);
                    const moveY = evt.movementY / (zoom || 1);
                    return {
                        ...l,
                ...
                const nextPos = clampDraggedLayerPosition({
                    layer: l,
                    pointerEvent: evt,
                    rect,
                    offset
                ...
                return { ...l, x: nextPos.x, y: nextPos.y };
            }));
        };
        ...
        const handleGlobalMouseMove = (e) => {
            ...
            const now = Date.now();
            if (shouldBroadcastPresence(now, lastPresenceUpdate.current, 120) && designId && user?.uid) {
            ...
            if (dragRef.current.isDragging && dragRef.current.layerId) {
                latestDragEventRef.current = e;
                if (!dragMoveRafRef.current) {
                    dragMoveRafRef.current = requestAnimationFrame(applyDragFrame);
                }
                return;
    ...
    }, [isDraggingLayer, resizing, handleMouseUp, isPanningContent, zoom, designId, user?.uid, user?.displayName, activePageId, userColor, isResizingCanvas]);
    ...
    const TEXTABLE_SHAPES = ['square', 'square-rounded', 'circle', 'triangle', 'pentagon', 'hexagon', 'octagon', 'parallelogram', 'star-5'];

    const updateShapeTextContent = (layerId, content, saveHistoryEntry = false) => {
        setLayers(prev => {
            const nextLayers = prev.map(l => l.id === layerId ? { ...l, content } : l);
            saveEditorState({ layers: nextLayers, adjustments, canvasSize });
            if (saveHistoryEntry) saveToHistory(nextLayers);
            // Sync text changes to Firebase for real-time collaboration
            syncLayersToFirebase(nextLayers);
            return nextLayers;
        });
    };
    ...
    const addShape = (type, content = null) => {
        ...
        let width = Math.max(80, bgW * 0.28);
        let height = Math.max(80, bgH * 0.28);
        let color = '#9333ea';

        if (type.startsWith('line') || type === 'arrow') {
            width = bgW * 0.4;
            height = 14; // Compact default hit area height
            color = darkMode ? '#ffffff' : '#000000';
        } else if (isIcon) {
            width = Math.max(56, bgW * 0.16);
            height = width;
            color = '#3b82f6';
        } else if (isGradient) {
            width = Math.max(110, bgW * 0.34);
            height = Math.max(80, bgH * 0.24);
            color = content;
        }
    ...
    const renderFinalCanvas = async (customLayers = null, customAdjustments = null, options = {}) => {
        ...
        // Allow per-page rendering (presentation/viewer) while keeping current canvas fallback.
        const targetCanvasSize = options.canvasSize || canvasSize;
        const baseWidth = targetCanvasSize.width;
        const baseHeight = targetCanvasSize.height;
        ...
        for (const layer of targetLayers) {
            ...
            if (layer.id === 'background-layer' || (layer.type === 'shape' && layer.shapeType === 'image')) {
                ...
                if (layer.shapeType === 'icon') {
                    ...
                    const isTextableShape = TEXTABLE_SHAPES.includes(type);
                    ...
                    // Keep user-entered text inside supported shapes in exported image.
                    const rawShapeText = typeof layer.content === 'string' ? layer.content.trim() : '';
                    if (isTextableShape && rawShapeText) {
                        const fontSize = Math.max(11 * finalScale, Math.min(w, h) * 0.15);
                        const lineHeight = fontSize * 1.2;
                        const maxTextWidth = Math.max(20 * finalScale, w * 0.75);
                        const words = rawShapeText.split(/\s+/).filter(Boolean);
                        const lines = [];
                        let currentLine = '';
    ...
    */
    ...
    const TabButton = ({ id, icon: Icon, label, premium }) => (
        <button
            ...
            className={`group relative flex flex-col items-center justify-center w-full py-1.5 transition-all duration-200 ${activeTab === id && isPanelOpen
                ? (darkMode ? 'bg-gray-800 text-white shadow-[inset_0_1px_1px_rgba(255,255,255,0.05)]' : 'bg-sky-100 text-sky-700 shadow-[0_10px_25px_rgba(15,23,42,0.08)]')
                : (darkMode ? 'text-gray-400 hover:text-white' : 'text-slate-700 hover:text-sky-700')
                }`}
        >
            <div className={`p-1.5 rounded-lg transition-all duration-200 ${activeTab === id && isPanelOpen ? 'bg-cyan-500/10' : (darkMode ? 'group-hover:bg-gray-100/50' : 'group-hover:bg-slate-100')}`}>
    ...
    const renderPresentationHeader = () => (
        <header className="h-12 flex-shrink-0 flex items-center justify-between px-6 bg-slate-950/90 backdrop-blur-md border-b border-white/5 z-[100]">
    ...
    const renderPresentationFooter = () => {
        ...
        return (
            <div className="h-14 flex-shrink-0 flex items-center justify-between px-8 bg-slate-950/80 backdrop-blur-md border-t border-white/10 z-[100] shadow-2xl">
    ...
    return (
        <div className={`fixed inset-0 z-[100] flex flex-col h-screen ${darkMode ? 'bg-[radial-gradient(circle_at_top_left,_rgba(6,182,212,0.10),_transparent_30%),radial-gradient(circle_at_top_right,_rgba(59,130,246,0.12),_transparent_32%),linear-gradient(135deg,_#050816_0%,_#0b1220_48%,_#0f172a_100%)] text-white' : 'bg-[radial-gradient(circle_at_top_left,_rgba(6,182,212,0.08),_transparent_26%),radial-gradient(circle_at_top_right,_rgba(59,130,246,0.10),_transparent_30%),linear-gradient(135deg,_#ecfeff_0%,_#f8fafc_40%,_#e2e8f0_100%)] text-gray-900'} transition-all duration-500 overflow-hidden text-sm antialiased`}>
            {/* TOP NAVIGATION BAR */}
            {isViewOnly ? renderPresentationHeader() : (
                <header className={`h-14 sm:h-16 flex-shrink-0 flex items-center justify-between px-3 sm:px-4 lg:px-6 border-b ${darkMode ? 'bg-slate-950/88 border-slate-800/90' : 'bg-white/92 border-slate-200'} backdrop-blur-2xl z-[60] shadow-[0_14px_40px_rgba(15,23,42,0.22)] dark:shadow-[0_20px_50px_rgba(2,8,23,0.45)]`}>
                    <div className="flex items-center gap-2 sm:gap-3 md:gap-4">
                        {(isOwner || !designId) && (
                            <button
                                onClick={handleClose}
                                className={`p-2 rounded-xl transition-all duration-200 ${darkMode ? 'hover:bg-gray-800 text-gray-400 hover:text-white hover:scale-105' : 'hover:bg-slate-100 text-slate-500 hover:text-slate-900 hover:scale-105'}`}
                                title="Close Editor"
                            >
                        ...
                        <div className="flex items-center gap-2 sm:gap-2.5">
                            <div className="w-8 h-8 sm:w-9 sm:h-9 bg-gradient-to-br from-cyan-500 via-sky-500 to-blue-600 rounded-2xl flex items-center justify-center shadow-lg shadow-cyan-500/30 hover:shadow-cyan-500/40 transition-all duration-300 hover:scale-105">
                                <Sparkles className="w-4 h-4 sm:w-5 sm:h-5 text-white fill-white/20" />
                            </div>
                            <div className="hidden sm:block">
                                <span className={`font-black tracking-tight text-sm lg:text-base bg-gradient-to-r from-cyan-300 via-sky-300 to-blue-400 bg-clip-text text-transparent`}>Editor <span className={`${darkMode ? 'text-cyan-300' : 'text-cyan-300'}`}>PRO</span></span>
                                <p className={`text-[9px] font-medium ${darkMode ? 'text-slate-500' : 'text-slate-500'} -mt-0.5`}>Professional Design Studio</p>
                            </div>
                            <div className="sm:hidden">
                                <span className={`font-black tracking-tight text-sm bg-gradient-to-r from-cyan-300 to-blue-400 bg-clip-text text-transparent`}>Editor</span>
                            </div>
                        </div>
                        ...
                        <div className="flex items-center gap-0.5 sm:gap-1 hidden lg:flex">
                            <button
                                ...
                                className={`p-2 rounded-xl transition-all duration-200 ${historyIndex < 0 ? 'opacity-30 cursor-not-allowed' : (darkMode ? 'hover:bg-gray-800 text-gray-400 hover:text-white hover:scale-105' : 'hover:bg-slate-100 text-slate-500 hover:text-slate-900 hover:scale-105')}`}
                                title="Undo (Ctrl+Z)"
                            >
                            ...
                            <button
                                ...
                                className={`p-2 rounded-xl transition-all duration-200 ${historyIndex >= history.length - 1 ? 'opacity-30 cursor-not-allowed' : (darkMode ? 'hover:bg-gray-800 text-gray-400 hover:text-white hover:scale-105' : 'hover:bg-slate-100 text-slate-500 hover:text-slate-900 hover:scale-105')}`}
                                title="Redo (Ctrl+Y)"
                            >
                    ...
                    <div className="flex items-center gap-1.5 sm:gap-2 md:gap-3">
                        {!isViewOnly && (
                            <>
                                ...
                                <button
                                    onClick={handleDownload}
                                    className={`sm:hidden p-2 rounded-2xl transition-all duration-200 hover:scale-105 ${darkMode ? 'bg-slate-900/90 hover:bg-slate-800 text-white border border-slate-800' : 'bg-white text-slate-700 hover:bg-slate-100 border border-slate-200'}`}
                                ...
                                <button
                                    ...
                                    className="flex items-center gap-2 px-3 sm:px-4 py-2 bg-gradient-to-r from-cyan-500 via-sky-500 to-blue-600 hover:from-cyan-600 hover:via-sky-600 hover:to-blue-700 text-white rounded-2xl text-xs font-bold transition-all duration-200 shadow-lg shadow-cyan-500/30 hover:shadow-cyan-500/50 active:scale-95 hover:scale-105 transform"
                                >
            ...
            <ExportManager
                ...
                onDesignIdGenerated={(id) => {
                    setDesignId(id);
                    hasLoadedInitialData.current = true; // Owner has the data, allow broadcasting
                    console.log("[FirebaseSync] 📤 Project shared! Enabled broadcasting.");
                }}
                user={user}
            ...
            <div className="flex flex-1 overflow-hidden relative">
                ...
                {!isViewOnly && (
                    <div className={`w-16 sm:w-[72px] h-full flex-shrink-0 flex flex-col items-center py-3 sm:py-4 border-r ${darkMode ? 'bg-slate-950/82 border-slate-800/85' : 'bg-white/92 border-slate-200'} backdrop-blur-2xl z-50 shadow-xl shadow-gray-200/30 dark:shadow-[0_24px_70px_rgba(2,8,23,0.28)]`}>
                        ...
                        <button
                            ...
                            className={`mt-auto mb-2 p-2 rounded-2xl transition-all duration-200 ${isPanelOpen ? 'bg-cyan-500 text-white shadow-lg shadow-cyan-500/30' : (darkMode ? 'bg-slate-900 text-slate-400 hover:text-white' : 'bg-white text-slate-500 hover:text-slate-800 border border-slate-200')}`}
                ...
                <div
                    className={`${isPanelOpen
                        ...
                        } h-full flex-shrink-0 transition-all duration-300 ease-out border-r ${darkMode ? 'bg-slate-950/80 border-slate-800/85' : 'bg-white/88 border-slate-200/70'} backdrop-blur-2xl z-20 overflow-y-auto custom-scrollbar relative shadow-2xl shadow-gray-200/50 dark:shadow-[0_30px_80px_rgba(2,8,23,0.34)]`}
                >
                    <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-cyan-500 via-sky-500 to-blue-600" />
                                    ...
                                    {[
                                        ...
                                        >
                                            ...
                                            <div>
                                                ...
                                                <p className={`text-[9px] ${darkMode ? 'text-gray-600' : 'text-slate-600'} mt-0.5`}>{template.desc}</p>
                                            </div>
                                        </button>
                                    ))}
                                </div>
                            ...
                            <div className="animate-fadeIn space-y-6">
                                ...
                                <div className="space-y-8">
                                    {Object.entries(imageAnimations).map(([category, templates]) => (
                                        <div key={category}>
                                            <h4 className={`text-[10px] font-black uppercase mb-3 ${darkMode ? 'text-gray-500' : 'text-slate-600'}`}>
                                                {category}
                                            </h4>
                                            <div className="grid grid-cols-3 gap-2">
                                                {templates.map((template) => {
                                                    ...
                                                    return (
                                                        ...
                                                        >
                                                            <div className={`w-10 h-10 rounded-lg flex items-center justify-center shadow-sm ${darkMode ? 'bg-gray-800 text-purple-400' : 'bg-sky-50 text-sky-700'}`}>
                                                                <IconCmp className="w-5 h-5" />
                                                            </div>
                                                            <span className={`text-[9px] font-bold text-center truncate w-full ${darkMode ? 'text-gray-300' : 'text-slate-700'}`}>
                                                                {template.name}
                                                            </span>
                                                        </button>
                                                    );
                                        ...
                                        <div className="animate-fadeIn space-y-6">
                                            ...
                                            <div>
                                                ...
                                                <div className="grid grid-cols-4 gap-4 items-center px-2">
                                                    <button onClick={() => addShape('line')} className="h-0.5 bg-slate-950 dark:bg-white w-full rounded-full" />
                                                    ...
                                                    <button onClick={() => addShape('arrow')} className="flex items-center w-full">
                                                        <div className="h-0.5 bg-slate-950 dark:bg-white flex-1" />
                                            ...
                                            <div>
                                                ...
                                                <div className="grid grid-cols-4 gap-3 place-items-center">
                                                    <button onClick={() => addShape('square')} className="w-10 h-10 bg-slate-950 dark:bg-white rounded-sm hover:scale-110 transition-transform" />
                                                    <button onClick={() => addShape('square-rounded')} className="w-10 h-10 bg-slate-950 dark:bg-white rounded-xl hover:scale-110 transition-transform" />
                                                    <button onClick={() => addShape('circle')} className="w-10 h-10 bg-slate-950 dark:bg-white rounded-full hover:scale-110 transition-transform" />
                                            ...
                                            <div>
                                                ...
                                                <div className="grid grid-cols-4 gap-3 place-items-center">
                                                    ...
                                                    <button onClick={() => addShape('parallelogram')} className="w-10 h-10 bg-slate-950 dark:bg-white -skew-x-12 rounded-sm hover:scale-110 transition-transform" />
                                                </div>
                                            </div>
                                            {/* Stars */}
                                            <div>
                                        ...
                                        <div className="grid grid-cols-4 gap-2">
                                            {['#9333ea', '#6366f1', '#f472b6', '#fbbf24', '#10b981', '#ef4444', '#3b82f6', '#000000'].map(color => (
                                                <button
                                                    ...
                                                    onClick={() => {
                                                        if (activeLayerId) {
                                                            const nextLayers = layers.map(l => l.id === activeLayerId ? { ...l, color } : l);
                                                            setLayers(nextLayers);
                                                            saveToHistory(nextLayers);
                                                            syncLayersToFirebase(nextLayers);
                                                        }
                                            ...
                                            .map(project => (
                                                ...
                                                >
                                                    ...
                                                    {projectMenuOpen === project.id && (
                                                        <div
                                                            className={`absolute top-10 right-2 ${darkMode ? 'bg-slate-950 border-gray-700' : 'bg-white border-gray-200'} border rounded-lg shadow-xl z-50 overflow-hidden`}
                ...
                {/* SAVING OVERLAY */}
                {isSaving && (
                    <div className="absolute inset-0 z-[100] flex items-center justify-center bg-slate-950/80 backdrop-blur-sm animate-fadeIn">
                        <div className={`p-8 rounded-3xl ${darkMode ? 'bg-slate-950 border-gray-800' : 'bg-white border-gray-100'} border-2 shadow-2xl flex flex-col items-center gap-4 max-w-xs w-full`}>
                            <div className="relative">
                ...
                {isUploading && (
                    <div className="absolute inset-0 z-[100] flex items-center justify-center bg-white/40 dark:bg-black/40 animate-fadeIn">
                        <div className={`p-8 rounded-3xl ${darkMode ? 'bg-slate-950 border-gray-800' : 'bg-white border-gray-100'} border-2 shadow-2xl flex flex-col items-center gap-4 max-w-xs w-full backdrop-blur-md`}>
                            <div className="relative">
                ...
                {/* MAIN CANVAS AREA */}
                <div
                    className={`flex-1 relative flex flex-col ${isViewOnly ? 'bg-[#020617]' : (darkMode ? 'bg-gradient-to-br from-[#020617] via-[#0f172a] to-[#020617]' : 'bg-gradient-to-br from-slate-100 via-gray-50 to-slate-200')} transition-all overflow-hidden`}
                ...
                >
                    {/* Workspace (Canvas) */}
                    <div className={`flex-1 relative overflow-hidden flex items-center justify-center ${isViewOnly ? 'p-0' : 'p-3 sm:p-4 lg:p-6 xl:p-8 pb-16 sm:pb-20'} ${darkMode ? 'bg-slate-950/20' : 'bg-gray-200/50'}`}>
                        ...
                        {isSyncing && (
                            <div className="absolute inset-0 bg-slate-950/60 backdrop-blur-md z-[100] flex flex-col items-center justify-center transition-all duration-500 animate-fadeIn">
                                            ...
                                            {(() => {
                                                ...
                                                return (
                                                    ...
                                                    >
                                                        ...
                                                        <div
                                                            className={`absolute inset-0 bg-slate-950/80 z-10 pointer-events-none ${isBackground ? '' : 'rounded-sm'}`}
                                                            style={{
                                                                clipPath: `polygon(
                                                                    0% 0%, 0% 100%,
                                ...
                                >
                                    {layer.type === 'frame' ? (
                                        ...
                                        <div
                                            ...
                                            title={editingLayerId === layer.id ? 'Editing text (hold Alt + drag to move)' : 'Double click to edit text'}
                                            onBlur={(e) => {
                                        ...
                                        >
                                            ...
                                            {(() => {
                                                ...
                                                const isTextableShape = TEXTABLE_SHAPES.includes(layer.shapeType);
                                                    ...
                                                    >
                                                        {isTextableShape && (
                                                            <div
                                                                ...
                                                                suppressContentEditableWarning
                                                                onInput={(e) => updateShapeTextContent(layer.id, e.currentTarget.innerText, false)}
                                                                onBlur={(e) => {
                                                                    updateShapeTextContent(layer.id, e.currentTarget.innerText, true);
                                                                }}
                                                                onMouseDown={(e) => e.stopPropagation()}
                ...
                {!isViewOnly && isGridView && (
                    <div className="absolute inset-0 z-[200] bg-gray-50/95 dark:bg-slate-950/95 backdrop-blur-md animate-fadeIn flex flex-col">
                        {/* Grid View Header */}
                                ...
                                {pages.map((page, index) => (
                                    <div key={page.id} className="flex flex-col gap-2 group">
                                        ...
                                        >
                                            ...
                                            {/* Page Number Badge */}
                                            <div className="absolute top-2 left-2 bg-slate-950/80 backdrop-blur-md text-white px-2 py-0.5 rounded text-xs font-bold">
                                                {index + 1}
                                            </div>
                ...
                {!isViewOnly && (
                    <div className={`absolute bottom-3 sm:bottom-4 right-3 sm:right-4 lg:right-6 z-50 flex items-center gap-2 sm:gap-3 ${darkMode ? 'bg-gray-800/95 border-gray-700/80' : 'bg-white/95 border-gray-200/80'} backdrop-blur-xl p-2 sm:p-2.5 rounded-2xl shadow-2xl border transition-all duration-300 hover:shadow-purple-500/10`}>
                        ...
                        >
                            ...
                            <span className={`${darkMode ? 'bg-slate-950 text-gray-300' : 'bg-gray-200 text-gray-600'} px-1.5 py-0.5 rounded text-[10px] font-bold`}>
